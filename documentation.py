...
def add_to_AQ_database(data, location_id, month, cur, conn):
    ''' Takes in data, location_id, month, database cursor and connector as inputs. 
    Goes through api data by day. Records date, month, location, and air quality 
    average. Inserts data into Air_Quality table and commits to database 
    connector.'''
   def API_driver(location, month, cur, conn):
    ''' Takes in location, month, database cursor and connector as inputs. Calls get 
    monthly cases to get API data. Checks for errors in API response. If there are no 
    errors, calls add_to_AQ_database and returns a tuple with just True. If there are 
    errors, returns a tuple with False and the error message.'''
